        ...
        await new Promise(async (resolve, reject) => {
            await fs.readFile(__dirname + `/../img/${name}.png`, async function (err, data) {
                ...
                for (let y = 0; y < img.height; y++) {
                    ...
                    if (!isImage && isBG) {
                        if (!cropAreaArray[cropAreaArray.length - 1] || y - cropAreaArray[cropAreaArray.length - 1][0] > 0) {
                            cropArea.push(y);
                            isImage = true;
                        }
                    }
                ...
                for (let i = 0; i < splitImages.length; i++) {
                    fs.readFile(__dirname + `/../img/${name}/${splitImages[i]}`, function (err, data) {
                        ...
                        for (let x = 0; x < img.width; x++) {
                            ...
                            if (!isImage && isBG) {
                                if (!cropAreaArray[cropAreaArray.length - 1] || x - cropAreaArray[cropAreaArray.length - 1][0] > 0) {
                                    cropArea.push(x);
                                    isImage = true;
                                }
                            }
